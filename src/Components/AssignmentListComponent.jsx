...
export default function AssignmentListComponent(props) {
    ...
    return (
        <>
        <div className={"assignment-list-div"} onClick={() => setIsOpen(true)}>
            <div className={"assignment-header-list-div"}>
                <p className={"assignment-header-list-text"}>
                    {props.assignment.name}
                </p>
                <p className={"assignment-header-list-subtext"}>
                    {courses.find(course => course.id === props.assignment.course_id).name}
                </p>
            </div>
            <div className={"assignment-body-list-div"}>
                { props.assignment.due_at != null ? <ChipFieldComponent className={"float-right"} title={deadlineDate.toISOString().split(/[T ]/i, 1)[0]} color={deadlineColor(props.assignment.dueDate)}/> : <></>}
                {tasks.length > 0 ? 
