...
export default function ProgressBarComponent(props) {
    ...
    return(
        <div className={"progress-bar-div"}>
            ...
            <div className={"progress-bar bg-[#F5F5F5]"} style={{width: width + "rem"}}>
                <div className={`h-1 rounded-full ${(width / 100 * props.progress) === 100 ? "bg-[#F5761A]" : "bg-[#F5761A]"}`} style={{width: (width / 100 * props.progress) + "rem"}}></div>
            </div>
        </div>
    )
}